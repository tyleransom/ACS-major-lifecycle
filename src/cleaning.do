...
quietly infix                 ///
  ...
  byte    educ       107-108  ///
  int     educd      109-111  ///
  byte    degfield   112-113  ///
  int     degfieldd  114-117  ///
  byte    empstat    118-118  ///
  byte    empstatd   119-120  ///
  byte    labforce   121-121  ///
  byte    classwkr   122-122  ///
  byte    classwkrd  123-124  ///
  int     occ2010    125-128  ///
  byte    wkswork2   129-129  ///
  byte    uhrswork   130-131  ///
  byte    workedyr   132-132  ///
  long    incwage    133-138  ///
  byte    qincwage   139-139  ///
  using ${datpath}usa_00053.dat
...
* make a crosswalk between ACS and CIP major codes
* see https://nces.ed.gov/ipeds/cipcode/browse.aspx?y=56 for CIP codes
* see downloaded spreadsheet for crosswalk

generat cip2d =  .
replace cip2d =  1 if inlist(degfield, 11) // Ag Sci
replace cip2d =  3 if inlist(degfield, 13) // Natural Resources/Conservation
replace cip2d =  4 if inlist(degfield, 14) // Architecture
replace cip2d =  5 if inlist(degfield, 15) // Area Studies
replace cip2d =  9 if inlist(degfield, 19) // Communications
replace cip2d = 10 if inlist(degfield, 20) // Communications Technologies
replace cip2d = 11 if inlist(degfield, 21) // Computer and Information Sciences
replace cip2d = 12 if inlist(degfield, 22) // Cosmetology
replace cip2d = 13 if inlist(degfield, 23) // Education
replace cip2d = 14 if inlist(degfield, 24) // Engineering
replace cip2d = 15 if inlist(degfield, 25) // Engineering Technologies
replace cip2d = 16 if inlist(degfield, 26) // Foreign Languages
replace cip2d = 19 if inlist(degfield, 29) // Family and Consumer Sciences
replace cip2d = 22 if inlist(degfield, 32) // Legal Studies
replace cip2d = 23 if inlist(degfield, 33) // English
replace cip2d = 24 if inlist(degfield, 34) // Liberal Arts
replace cip2d = 25 if inlist(degfield, 35) // Library Science
replace cip2d = 26 if inlist(degfield, 36) // Biological Sciences
replace cip2d = 27 if inlist(degfield, 37) // Mathematics
replace cip2d = 28 if inlist(degfield, 38) // Military Science
replace cip2d = 30 if inlist(degfield, 40) // Multi/Interdisciplinary Studies
replace cip2d = 31 if inlist(degfield, 41) // Parks and Recreation
replace cip2d = 38 if inlist(degfield, 48) // Philosophy and Religion
replace cip2d = 39 if inlist(degfield, 49) // Theology
replace cip2d = 40 if inlist(degfield, 50) // Physical Sciences
replace cip2d = 41 if inlist(degfield, 51) // Science Technologies
replace cip2d = 42 if inlist(degfield, 52) // Psychology
replace cip2d = 43 if inlist(degfield, 53) // Homeland Security
replace cip2d = 44 if inlist(degfield, 54) // Public Administration
replace cip2d = 45 if inlist(degfield, 55) // Social Sciences
replace cip2d = 46 if inlist(degfield, 56) // Construction Trades
replace cip2d = 47 if inlist(degfield, 57) // Mechanic and Repair Technologies
replace cip2d = 48 if inlist(degfield, 58) // Precision Production
replace cip2d = 49 if inlist(degfield, 59) // Transportation and Materials Moving
replace cip2d = 50 if inlist(degfield, 60) // Visual and Performing Arts
replace cip2d = 51 if inlist(degfield, 61) // Health
replace cip2d = 52 if inlist(degfield, 62) // Business
replace cip2d = 54 if inlist(degfield, 64) // History
replace cip2d = 55 if mi(cip2d)            // Other/Missing

lab def vlcip2d 1 "Agricultural/Animal/Plant/Veterinary Science and Related Fields" 3 "Natural Resources and Conservation" 4 "Architecture and Related Services" 5 "Area, Ethnic, Cultural, Gender, and Group Studies" 9 "Communication, Journalism, and Related Programs" 10 "Communications Technologies/Technicians and Support Services" 11 "Computer and Information Sciences and Support Services" 12 "Culinary, Entertainment, and Personal Services" 13 "Education" 14 "Engineering" 15 "Engineering/Engineering-Related Technologies/Technicians" 16 "Foreign Languages, Literatures, and Linguistics" 19 "Family and Consumer Sciences/Human Sciences" 21 "Reserved" 22 "Legal Professions and Studies" 23 "English Language and Literature/Letters" 24 "Liberal Arts and Sciences, General Studies and Humanities" 25 "Library Science" 26 "Biological and Biomedical Sciences" 27 "Mathematics and Statistics" 28 "Military Science, Leadership and Operational Art" 29 "Military Technologies and Applied Sciences" 30 "Multi/Interdisciplinary Studies" 31 "Parks, Recreation, Leisure, Fitness, and Kinesiology" 32 "Basic Skills and Developmental/Remedial Education" 33 "Citizenship Activities" 34 "Health-Related Knowledge and Skills" 35 "Interpersonal and Social Skills" 36 "Leisure and Recreational Activities" 37 "Personal Awareness and Self-Improvement" 38 "Philosophy and Religious Studies" 39 "Theology and Religious Vocations" 40 "Physical Sciences" 41 "Science Technologies/Technicians" 42 "Psychology" 43 "Homeland Security, Law Enforcement, Firefighting and Related Protective Services" 44 "Public Administration and Social Service Professions" 45 "Social Sciences" 46 "Construction Trades" 47 "Mechanic and Repair Technologies/Technicians" 48 "Precision Production" 49 "Transportation and Materials Moving" 50 "Visual and Performing Arts" 51 "Health Professions and Related Programs" 52 "Business, Management, Marketing, and Related Support Services" 53 "High School/Secondary Diplomas and Certificates" 54 "History" 55 "Missing/Other"
lab val cip2d vlcip2d

generat cipH =  .
replace cipH =  1 if inlist(degfield, 11)       // Agriculture
replace cipH =  2 if inlist(degfield, 13)       // Environmental and Natural Resources
replace cipH =  3 if inlist(degfield, 14)       // Architecture
replace cipH =  4 if inlist(degfield, 15)       // Area, Ethnic, and Civ Studies
replace cipH =  5 if inlist(degfield, 19,20)    // Communications and Comm Tech
replace cipH =  6 if inlist(degfield, 21)       // Computer and Information Sciences
replace cipH =  7 if inlist(degfield, 23)       // Education Administration and Teaching
replace cipH =  8 if inlist(degfield, 24)       // Engineering
replace cipH =  9 if inlist(degfield, 25)       // Engineering Technologies
replace cipH = 10 if inlist(degfield, 26)       // Linguistics and Foreign Languages
replace cipH = 11 if inlist(degfield, 29)       // Family and Consumer Sciences
replace cipH = 12 if inlist(degfield, 32)       // Law
replace cipH = 13 if inlist(degfield, 33)       // English Lang, Lit, and Comp
replace cipH = 14 if inlist(degfield, 34,35)    // Liberal Arts and Humanities (and Lib Sci)
replace cipH = 15 if inlist(degfield, 36)       // Biological Sciences
replace cipH = 16 if inlist(degfield, 37)       // Mathematics and Statistics
replace cipH = 17 if inlist(degfield, 22,38,40) // Interdisc and Mulit-Disc Studies (and military and cosmetology)
replace cipH = 18 if inlist(degfield, 41)       // Physicall Fitness and Leisure
replace cipH = 19 if inlist(degfield, 48)       // Philosophy and Religious Studies
replace cipH = 20 if inlist(degfield, 49)       // Theology and Religious Vocations
replace cipH = 21 if inlist(degfield, 50,51)    // Physical Sciences and Technologies
replace cipH = 22 if inlist(degfield, 52)       // Psychology
replace cipH = 23 if inlist(degfield, 53)       // Criminal Justice and Fire Protection
replace cipH = 24 if inlist(degfield, 54)       // Public Affairs, Policy, and Social Work
replace cipH = 25 if inlist(degfield, 55)       // Social Sciences
replace cipH = 26 if inlist(degfield, 56,57,58) // Construction/Mechanic/Production Tech
replace cipH = 27 if inlist(degfield, 59)       // Transportation Sciences and Technologies
replace cipH = 28 if inlist(degfield, 60)       // Fine Arts
replace cipH = 29 if inlist(degfield, 61)       // Medical and Health Sciences and Services
replace cipH = 30 if inlist(degfield, 62)       // Business
replace cipH = 31 if inlist(degfield, 64)       // History
replace cipH = 55 if mi(cip2d)                  // Other/Missing

lab def vlcipH 1 "Agriculture" 2 "Environmental and Natural Resources" 3 "Architecture" 4 "Area, Ethnic, and Civ Studies" 5 "Communications and Comm Tech" 6 "Computer and Information Sciences" 7 "Education" 8 "Engineering" 9 "Engineering Technologies" 10 "Linguistics and Foreign Languages" 11 "Family and Consumer Sciences" 12 "Law" 13 "English Lang, Lit, and Comp" 14 "Liberal Arts and Humanities (and Lib Sci)" 15 "Biology" 16 "Mathematics and Statistics" 17 "Interdisc and Mulit-Disc Studies (and military and cosmetology)" 18 "Physicall Fitness and Leisure" 19 "Philosophy and Religious Studies" 20 "Theology and Religious Vocations" 21 "Physical Sciences and Technologies" 22 "Psychology" 23 "Criminal Justice and Fire Protection" 24 "Public Affairs, Policy, and Social Work" 25 "Social Sciences" 26 "Construction/Mechanic/Production Tech" 27 "Transportation Sciences and Technologies" 28 "Fine Arts" 29 "Medical and Health Sciences and Services" 30 "Business" 31 "History" 55 "Other/Missing"
lab val cipH vlcipH

* alternative defintion combining CIP 2- and 4-digit (AKS)
generat majAKS = .
replace majAKS = 1  if inlist(degfieldd,3702,3700,3701,4005)
replace majAKS = 2  if inlist(degfieldd,5007 )
replace majAKS = 3  if inlist(degfieldd,2410,2411,2415,2416,2417,2418,2419,2499,2400,2401,2402,2403,2404,2412,2413,2501,2407)
replace majAKS = 4  if inlist(degfieldd,2414)
replace majAKS = 5  if inlist(degfieldd,2408)
replace majAKS = 6  if inlist(degfieldd,5003)
replace majAKS = 7  if inlist(degfieldd,2102,2100,2105,2106,2107)
replace majAKS = 8  if inlist(degfieldd,2406)
replace majAKS = 9  if inlist(degfieldd,2405)
replace majAKS = 10 if inlist(degfieldd,2502,2503,2504,2599,2500,5102)
replace majAKS = 11 if inlist(degfieldd,5000,5005,5006,5004,5002,5001,2409)
replace majAKS = 12 if inlist(degfieldd,2309)
replace majAKS = 13 if inlist(degfieldd,2101)
replace majAKS = 14 if inlist(degfieldd,3699,3609,3606,3603,3602,3604,3600,3605,3601,4006,3607,3608,4003)
replace majAKS = 15 if inlist(degfieldd,5801,5601,5701,6000,5901)
replace majAKS = 16 if inlist(degfieldd,5501)
replace majAKS = 17 if inlist(degfieldd,6207 )
replace majAKS = 18 if inlist(degfieldd,1301,1302,1303)
replace majAKS = 19 if inlist(degfieldd,6104,6106,6299,6200,6204,6205,6210)
replace majAKS = 20 if inlist(degfieldd,4008)
replace majAKS = 21 if inlist(degfieldd,6201,6202,6212)
replace majAKS = 22 if inlist(degfieldd,6206)
replace majAKS = 23 if inlist(degfieldd,1401)
replace majAKS = 24 if inlist(degfieldd,2313,2311,2306,2305,3501,2300,2301,2304,2307,2310,2312,2399,2314,2308)
replace majAKS = 25 if inlist(degfieldd,6203)
replace majAKS = 26 if inlist(degfieldd,1100,1101,1102,1103,1105,1106,1199,1104)
replace majAKS = 27 if inlist(degfieldd,2901)
replace majAKS = 28 if inlist(degfieldd,6004)
replace majAKS = 29 if inlist(degfieldd,5200,5201,5202,5203,5205,5206,5299)
replace majAKS = 30 if inlist(degfieldd,2602,2603)
replace majAKS = 31 if inlist(degfieldd,6002,6001 )
replace majAKS = 32 if inlist(degfieldd,4002,4101 )
replace majAKS = 33 if inlist(degfieldd,6211,2201)
replace majAKS = 34 if inlist(degfieldd,1901,1903,1904)
replace majAKS = 35 if inlist(degfieldd,6108,6103,6100,6199,6109,6102)
replace majAKS = 36 if inlist(degfieldd,5599,4007,5507,5500,5502,5504,5503)
replace majAKS = 37 if inlist(degfieldd,1501)
replace majAKS = 38 if inlist(degfieldd,3301,3401,3402,2601,3302)
replace majAKS = 39 if inlist(degfieldd,5506)
replace majAKS = 40 if inlist(degfieldd,5301)
replace majAKS = 41 if inlist(degfieldd,6402,6403)
replace majAKS = 42 if inlist(degfieldd,4001,5505)
replace majAKS = 43 if inlist(degfieldd,6000,6006)
replace majAKS = 44 if inlist(degfieldd,3202,5401,5402)
replace majAKS = 45 if inlist(degfieldd,5403,5404,6209)
replace majAKS = 46 if inlist(degfieldd,4801,4901)
replace majAKS = 47 if inlist(degfieldd,1902)
replace majAKS = 48 if inlist(degfieldd,6105)
replace majAKS = 49 if inlist(degfieldd,6110,2303)
replace majAKS = 50 if inlist(degfieldd,6003,6005,6007)
replace majAKS = 51 if inlist(degfieldd,6107)
replace majAKS = 34 if inlist(degfieldd,2001)
replace majAKS = 20 if inlist(degfieldd,5098,4000)
replace majAKS = 43 if inlist(degfieldd,6099)
replace majAKS = 3  if inlist(degfieldd,5008)
replace majAKS = 14 if inlist(degfieldd,3611)

lab def vlAKS  1  "Mathematics", add
lab def vlAKS  2  "Physics", add
lab def vlAKS  3  "All Other Engineering", add
lab def vlAKS  4  "Mechanical Engineering", add
lab def vlAKS  5  "Electrical Engineering", add
lab def vlAKS  6 "Chemistry", add
lab def vlAKS  7  "Computer and Info Tech", add
lab def vlAKS  8  "Civil Engineering", add
lab def vlAKS  9  "Chemical Engineering", add
lab def vlAKS  10  "Engineering Tech", add
lab def vlAKS  11  "Earth and Other Physical Sci", add
lab def vlAKS  12  "Secondary Education", add
lab def vlAKS  13  "Computer Programming", add
lab def vlAKS  14  "Biological Sciences", add
lab def vlAKS  15  "Precision Production & Industrial Arts", add
lab def vlAKS  16  "Economics", add
lab def vlAKS  17  "Finance", add
lab def vlAKS  18  "Environmental Studies", add
lab def vlAKS  19  "Misc. Business and Med. Support", add
lab def vlAKS  20  "Multidisciplinary or General Science", add
lab def vlAKS  21  "Accounting", add
lab def vlAKS  22  "Marketing", add
lab def vlAKS  23  "Architecture", add
lab def vlAKS  24  "Library Science and Education (Other)", add
lab def vlAKS  25  "Business Management and Administration", add
lab def vlAKS  26  "Agriculture and Agr. Science", add
lab def vlAKS  27  "Family and Consumer Science", add
lab def vlAKS  28  "Commercial Art and Design", add
lab def vlAKS  29  "Psychology", add
lab def vlAKS  30  "Foreign Language", add
lab def vlAKS  31  "Music and Speech/Drama", add
lab def vlAKS  32  "Fitness and Nutrition", add
lab def vlAKS  33  "Leisure Studies and Basic Skills", add
lab def vlAKS  34  "Communications", add
lab def vlAKS  35  "Other Med/Health Services", add
lab def vlAKS  36  "Other Social Science", add
lab def vlAKS  37  "Area, Ethnic, and Civ. Studies", add
lab def vlAKS  38  "Letters: Lit, Writing, Other", add
lab def vlAKS  39  "Political Science", add
lab def vlAKS  40  "Protective Services", add
lab def vlAKS  41  "History", add
lab def vlAKS  42  "International Relations", add
lab def vlAKS  43  "Art History and Fine Arts", add
lab def vlAKS  44  "Public Administration and Law", add
lab def vlAKS  45  "Social Work and Human Resources", add
lab def vlAKS  46  "Philosophy and Religion", add
lab def vlAKS  47  "Journalism", add
lab def vlAKS  48  "Medical Tech", add
lab def vlAKS  49  "Public Health (Physical and Mental)", add
lab def vlAKS  50  "Film and Other Arts", add
lab def vlAKS  51  "Nursing", add

lab val majAKS vlAKS


* foreign born status
gen foreign_born = inrange(bpl,100, .)

* full-time/full-year work status
gen ftfy = inrange(uhrswork,35,99) & inrange(wkswork,6,6)

* generate labor force status
gen employed = empstat==1 & ftfy==1

* bad earnings
gen bad_wage = qincwage != 0


save ${datpath}acs0919cleaned, replace
